...
public enum Sentencias {
    ...
    UPDATEPRODUCTO("UPDATE product SET nombre= ?, marca= ?,calorias= ?,Grasa_Toltal= ?,Grasas_Saturadas= ?, hidratos= ?,azucar= ?,proteina= ?,fibra= ?,sodio= ?,foto=? WHERE id= ?"),
    INSERTPRODUCTO("INSERT INTO product (nombre,marca,calorias,Grasa_Toltal,Grasas_Saturadas,hidratos,azucar,proteina,fibra,sodio,id,foto) VALUES(?,?,?,?,?,?,?,?,?,?,NULL,?)"),
    ...
    UPDATEEJERCICIO("UPDATE ejercicio SET Descripcion= ?, Foto_Ejer= ?,Grupo_Muscular= ?,Series= ?,Repeticiones= ?, Id= ?,name= ? WHERE id= ?"),
    DELETEEJERCICIO("DELETE FROM ejercicio WHERE id=?"),
    SELECTALLEJERCICIOS("SELECT * FROM ejercicio"),
    SELECTALLEJERCICIOSPORNOMBRE("SELECT * FROM ejercicio WHERE nombre like ?%"),
